...
1. **Traceability.** A donor (*muzakki*) cannot be told where their zakat went, and the *amil* cannot reconstruct it either.
...
4. **Access control.** Everyone with the sheet link can see everything — including donor NIK/phone numbers and beneficiary (*mustahik*) personal data.

BMIS is an internal web application that replaces the spreadsheet: it records collection (*penghimpunan*) and distribution (*penyaluran*), keeps per-fund balances correct, and gives the board a live dashboard — with role-based access enforced at the database layer.
...
| G5 | Usable in the field | An *amil* can record a cash donation on a phone in < 45 seconds |
...
- Full double-entry general ledger or PSAK 109 statutory financial statements. BMIS produces the *source data* for those, not the statements themselves.
...
Fund type drives *what the money may be spent on*. This is the sharia constraint the spreadsheet cannot enforce.
...
1. Is this a single Baitul Maal or multiple branches (*cabang*)? `branch_id` is stubbed but branch-scoped RLS is not specified.
