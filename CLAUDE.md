...
`/bantuan` renders `docs/ALUR-KERJA.md` by importing it with `?raw` (via the `@docs` alias) — deliberately not a copy, so the page and the document cannot diverge. Its heading ids are generated by `src/features/help/slug.ts`, which reimplements GitHub's slug rules so the document's own table of contents keeps working; `slug.test.ts` parses the real markdown and fails if any in-page anchor stops resolving. The route is lazy-loaded because it pulls react-markdown with it.
