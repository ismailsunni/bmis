        ...
        <>
          <div className="table-wrap">
            <table className="tbl">
              <thead>
                <tr>
                  <th>Kode</th><th>Nama</th><th>Tipe</th><th>Telepon</th>
                  <th>Kota</th><th>Terdaftar</th>
                </tr>
              </thead>
              <tbody>
                {data.rows.map((d) => (
...
function DonorForm({
  ...
  const [form, setForm] = useState({
    full_name: '', donor_type: 'individual' as DonorType, nik: '', npwp: '',
  ...
  const save = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['donors'] }); onClose() },
  })
  ...
  return (
    <Modal
      open={open} onClose={onClose} title="Donatur baru"
      footer={
        <>
    ...
    >
      <div className="space-y-3">
        ...
        <label className="flex items-center gap-2 text-sm">
          <input type="checkbox" className="h-4 w-4" checked={form.is_recurring}
...
function MergeDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const merge = useRpc<{ p_source: string; p_target: string; p_reason: string }>(
    'rpc_merge_donors', ['donors'],
  )

  return (
    <Modal
      open={open} onClose={onClose} title="Gabungkan donatur duplikat"
      footer={
        <>
          <Button variant="secondary" onClick={onClose}>Batal</Button>
          <Button
    ...
    >
      <div className="space-y-3">
        <p className="text-sm text-slate-500">
          Seluruh donasi dari donatur sumber dipindahkan ke donatur tujuan. Data sumber
