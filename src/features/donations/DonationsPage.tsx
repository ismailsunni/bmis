...
const statusTone: Record<DonationStatus, 'neutral' | 'success' | 'warning' | 'danger'> = {
  draft: 'neutral', pending: 'warning', verified: 'success',
  rejected: 'danger', voided: 'danger',
}
  ...
  return (
    <>
            ...
            {can.recordDonation(role) && (
              <>
                <Link to="/donasi/impor">
                  <Button variant="secondary" size="sm"><Upload size={16} /> Impor</Button>
                </Link>
      ...
      <div className="mb-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
        <Input placeholder="Cari kwitansi atau donatur"
        ...
        <Select value={filters.fundTypeId ?? ''}
        ...
        <div className="flex gap-2">
          <Input type="date" aria-label="Dari" onChange={(e) => set({ from: e.target.value || undefined })} />
          <Input type="date" aria-label="Sampai" onChange={(e) => set({ to: e.target.value || undefined })} />
        </div>
      </div>
        ...
        <>
          <div className="table-wrap">
            <table className="tbl">
              <thead>
                <tr>
                  <th>Kwitansi</th><th>Tanggal</th><th>Donatur</th><th>Jenis dana</th>
                  <th className="num">Jumlah</th><th>Metode</th><th>Status</th><th />
                </tr>
              </thead>
              <tbody>
                {data.rows.map((r) => (
                  <tr key={r.id}>
                    ...
                    <td><Badge tone={statusTone[r.status]}>{donationStatusLabels[r.status]}</Badge></td>
                    <td>
                      {r.status === 'verified' && (
                        <button
          ...
          <Pagination page={filters.page ?? 0} count={data.count}
                      onChange={(p) => setFilters((f) => ({ ...f, page: p }))} />
        </>
      ))}

      <DonationForm open={formOpen} onClose={() => setFormOpen(false)}
