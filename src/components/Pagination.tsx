...
export function Pagination({
  page, count, onChange,
}: { page: number; count: number; onChange: (p: number) => void }) {
    ...
    <div className="mt-3 flex items-center justify-between gap-2 text-sm text-slate-500">
      ...
      <div className="flex items-center gap-2">
        <Button size="sm" variant="secondary" disabled={page === 0}
                onClick={() => onChange(page - 1)}>Sebelumnya</Button>
        <span>{page + 1} / {pages}</span>
        <Button size="sm" variant="secondary" disabled={page + 1 >= pages}
                onClick={() => onChange(page + 1)}>Berikutnya</Button>
      </div>
    </div>
  )
}
